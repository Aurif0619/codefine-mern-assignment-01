...
export const Navbar = ({ isAuthenticated = false, cartItems = [] }: NavbarProps) => {
          ...
          <div className="hidden md:flex items-center space-x-6">
            <div className="relative">
              <div
                ...
                className="relative"
              >
                <Link to="/cart" className="flex items-center space-x-2 p-2 rounded-lg hover:bg-white/10 transition-all duration-200">
                  <div className="relative">
                    <svg 
                ...
                {isCartHovered && cartCount > 0 && (
                  <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border border-gray-100 p-4 z-50 animate-fadeIn">
                    <div className="flex justify-between items-center mb-4 pb-3 border-b">
                      <h4 className="font-bold text-lg text-gray-800">Shopping Cart</h4>
                      <span className="text-sm text-red-600 font-medium">{cartCount} items</span>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="font-bold text-lg text-red-600">${cartTotal.toFixed(2)}</span>
                      </div>
                      <Link 
                        to="/cart"
            ...
            {!isAuthenticated ? (
              <div className="flex items-center space-x-4">
                <Link to="/login">
                  <button 
                    className="px-5 py-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white rounded-full hover:from-yellow-500 hover:to-yellow-600 transition-all duration-300 font-medium shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                  >
                    Login
                  </button>
                </Link>
                <Link to="/signup">
                  <button 
                    className="px-5 py-2 border-2 border-yellow-300 text-yellow-300 rounded-full hover:bg-yellow-300 hover:text-red-600 transition-all duration-300 font-medium"
                  >
                    Signup
                  </button>
                </Link>
              ...
              {navLinks.map((link) => (
                ...
                {cartCount > 0 && (
                  <div className="mb-3 max-h-48 overflow-y-auto">
                    {cartItems.map((item) => (
                      <div key={item.id} className="flex items-center py-2 border-b border-red-100 last:border-0">
                ...
                <div className="text-sm text-gray-600 mb-2">
                  Total: <span className="font-bold text-red-600">${cartTotal.toFixed(2)}</span>
                </div>
              ...
              {!isAuthenticated ? (
                <div className="flex flex-col space-y-3 pt-4 border-t border-gray-100">
                  <Link to="/login" onClick={() => setIsMenuOpen(false)}>
                    <button 
                      className="w-full px-4 py-3 bg-gradient-to-r from-yellow-400 to-yellow-500 text-white rounded-lg hover:from-yellow-500 hover:to-yellow-600 transition-colors duration-200 font-medium shadow-lg"
                    >
                      Login
                    </button>
                  </Link>
                  <Link to="/signup" onClick={() => setIsMenuOpen(false)}>
                    <button 
                      className="w-full px-4 py-3 border-2 border-yellow-400 text-yellow-400 rounded-lg hover:bg-yellow-400 hover:text-white transition-colors duration-200 font-medium"
                    >
                      Signup
                    </button>
                  </Link>
                ...
                <div className="pt-4 border-t border-gray-100 space-y-3">
                  <div className="flex items-center space-x-3 px-4 py-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-red-600">User</p>
                    </div>
                  </div>
                  <button 
